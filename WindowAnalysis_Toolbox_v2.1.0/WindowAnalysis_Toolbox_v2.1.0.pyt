...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [Tool1, Tool2, Tool4, Tool5, Tool6, Tool7, Tool8, Tool9, Tool10, Tool11]
...
class Tool6(object):
    ...
    def execute(self, parameters, messages):
        ...
        include_population_density = parameters[2].value  # Get the optional boolean parameter

        # Folder name variations
        folder_variations = ["Accum", "Median", "Max", "Min"]
        
        # Flag to ensure the explanatory variables message is displayed only once
        explanatory_message_shown = False

        # Traverse Level A folder (Window_R_Input)
        for level_b_folder in os.listdir(input_WA_folder):
            level_b_path = os.path.join(input_WA_folder, level_b_folder)

            # Check if it's a directory (Level B)
            if os.path.isdir(level_b_path):
                arcpy.AddMessage(f"Working on {level_b_folder}")
                log_folder = os.path.join(level_b_path, "Log")

                if os.path.exists(log_folder):
                    arcpy.AddMessage(f"Processing Log folder at: {log_folder}")

                    output_level_b_folder = os.path.join(output_folder, level_b_folder)
                    if not os.path.exists(output_level_b_folder):
                        os.makedirs(output_level_b_folder)
                    ...
                    for variation in folder_variations:
                        ...
                        for file_name in file_list:
                            try:
                                ...
                                arcpy.AddError(f"Error: The file '{file_name}' is empty.")
                                continue
                            except pd.errors.ParserError:
                                arcpy.AddError(f"Error: The file '{file_name}' could not be parsed.")
                                continue

                            try:
                                temp_data_new = temp_data.iloc[:, 1:]  # Exclude the first column (pandas index)
                                ...
                                # Filter based on include_population_density
                                ...
                                explanatory_columns = [col for col in temp_data_filtered.columns if col != "disease"]

                                if not explanatory_message_shown:
                                    arcpy.AddMessage(f"Explanatory variables: {', '.join(explanatory_columns)}")
                                    explanatory_message_shown = True

                                if temp_data_filtered.shape[1] <= 1:
                                    arcpy.AddError(f"Error: Insufficient data columns for regression in '{file_name}'.")
                                    continue

                                try:
                                    temp_regression = sm.Logit(temp_data_filtered["disease"], temp_data_filtered.iloc[:, 1:]).fit()
                                except ValueError as e:
                                    arcpy.AddError(f"Error during logistic regression in file '{file_name}': {str(e)}.")
                                    continue
                                ...
                                try:
                                    ...
                                    arcpy.AddError(f"Error: Unable to write basic results to '{output_file_path}'.")
                                    continue

                                temp_regression_coefficient = temp_regression.summary2().tables[1]
                                temp_regression_coefficient["oddsRatio"] = np.exp(temp_regression_coefficient["Coef."])
                                ...
                                try:
                                    ...
                                    arcpy.AddError(f"Error: Unable to write coefficient results to '{output_file_path}'.")
                                    continue
    ...
    def execute(self, parameters, messages):
        ...
        for file_path in glob.glob(os.path.join(input_folder, '*.csv')):
            ...
            for index, row in log_mil_data_sorted.iterrows():
                # Assuming significance if any of the statistical columns is significant
